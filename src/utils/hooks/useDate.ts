...
export function useDate() {
  ...
  const day = date.toLocaleDateString(import.meta.env.VITE_LOCALE_STRING, {
    weekday: "long",
  });
  ...
  const month = date.toLocaleDateString(import.meta.env.VITE_LOCALE_STRING, {
    month: "long",
  });

  const dateFormatted = `${day}, ${month} ${dayNumber}`;

  const time = date.toLocaleTimeString(import.meta.env.VITE_LOCALE_STRING, {
    hour: "numeric",
    hour12: false,
    minute: "numeric",
  });
